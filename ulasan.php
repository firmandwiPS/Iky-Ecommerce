...
<nav x-data="{ atTop: true }"
    ...
    <div class="max-w-5xl mx-auto flex justify-between items-center px-4 py-3 text-blue-900">
        ...
        <div class="flex items-center gap-3">
            <!-- Keranjang -->
            <a href="keranjang.php" class="relative text-blue-800 hover:text-blue-900">
                <i class="fas fa-shopping-cart text-xl"></i>
                <?php if ($keranjang_total > 0): ?>
                    <span class="absolute -top-1 -right-2 bg-red-600 text-white text-xs font-semibold px-1.5 py-0.5 rounded-full">
                        <?= $keranjang_total ?>
                    </span>
                <?php endif; ?>
            </a>
...
<div class="pt-20 max-w-2xl mx-auto px-4 py-8">
    ...
    <?php else: ?>
        <div class="flex flex-col gap-4">
            <?php $i = 0; while ($ulasan = mysqli_fetch_assoc($query_ulasan)): $isRight = $i % 2 === 1; ?>
...
<div x-show="showModal" x-transition
    ...
    style="display: none;">
    <div class="bg-white w-full sm:w-[500px] p-6 rounded-t-2xl shadow-lg pb-20 overflow-y-auto max-h-screen"
        @click.away="showModal = false">
        <h3 class="text-lg font-semibold mb-4">Tulis Ulasan Anda</h3>
        <form method="POST" class="space-y-3">
            <select name="makanan_id" required class="w-full border px-3 py-2 rounded">
            ...
            <input type="text" name="nama_pengulas" placeholder="Nama Anda" required class="w-full border px-3 py-2 rounded">
            <textarea name="ulasan" placeholder="Ulasan Anda..." required rows="4" class="w-full border px-3 py-2 rounded resize-none"></textarea>
            <textarea name="kritik" placeholder="Kritik (opsional)" rows="2" class="w-full border px-3 py-2 rounded resize-none"></textarea>
            <textarea name="saran" placeholder="Saran (opsional)" rows="2" class="w-full border px-3 py-2 rounded resize-none"></textarea>
            ...
            <div class="flex justify-center gap-4 mt-6">
                <button type="button" @click="showModal = false" class="px-4 py-2 bg-red-600 text-white rounded">Batal</button>
                <button type="submit" name="kirim" class="px-4 py-2 bg-blue-900 text-white rounded">Kirim</button>
            </div>
        </form>
    </div>
</div>
